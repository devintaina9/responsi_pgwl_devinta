...
@section('script')
    ...
    <script>
        // Map
        var map = L.map('map').setView([-7.704721380705496, 110.60174857959342], 15);
       ...
       var om = L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png', {
        ...
        var polygon = L.geoJson(null, {
            onEachFeature: function(feature, layer) {
                var popupContent = "Name: " + feature.properties.name + "<br>" +
                    ...
                    '{{ csrf_field() }}' +
                    '{{ method_field('DELETE') }}' + "<button type='submit' class='btn btn-sm btn-danger' onClick='return confirm(\"Apakah anda yakin Ingin menghapusnya?\")'><i class='fa-solid fa-trash-can'></i> Delete</button>" +
                    "</form>" +
                    "</div>";
        ...
        fetch('{{ asset("storage/geojson/areaklatenshp2.geojson") }}')
          ...
          var wisataLayer = L.geoJson(null, {
            ...
            onEachFeature: function (feature, layer) {
                var content = "id: " + feature.properties.fid + "<br" + "kecamatan:" + feature.properties.REMARK + "<br" ;
                layer.on({
                    click: function (e) {
                        layer.bindPopup(content).openPopup();
                    },
        ...
        var overlayMaps = {
            ...
            "Polylines": polyline,
            "Polygons": polygon,
            "areaklatenshp2" : administrasiLayer,
            "wisataklatenshp" : wisataLayer,
